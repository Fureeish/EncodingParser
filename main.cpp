...
*/
bool is_ASCII(const uint8_t c) noexcept {
...
void add_char(const std::string &file_content, std::vector<uint32_t> &values,
    ...
    switch (number_of_bytes) {
        case 1: {
            ...
            uint8_t second = static_cast<uint8_t>(fc[index + 1]) & following_byte_mask;
        ...
        case 3: {
            ...
            uint8_t second  =static_cast<uint8_t>(fc[index + 1]) & following_byte_mask;
            uint8_t third   = static_cast<uint8_t>(fc[index + 2]) & following_byte_mask;
            uint8_t fourth  = static_cast<uint8_t>(fc[index + 3]) & following_byte_mask;
